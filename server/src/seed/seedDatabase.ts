...
export const seed = async () => {
  try {
    ...
    // --- SWAAD RESTAURANT ---
    const swaadSouthCategory = await MenuCategory.create({ name: 'South Indian Specials', section: 'SWAAD', sortOrder: 1 });
    const swaadDosaCategory = await MenuCategory.create({ name: 'Dosa Specialities', section: 'SWAAD', sortOrder: 2 });
    const swaadNorthCategory = await MenuCategory.create({ name: 'North Pulav & Biriyani', section: 'SWAAD', sortOrder: 3 });
    const swaadCurryCategory = await MenuCategory.create({ name: 'Vegetable Curries & Dal', section: 'SWAAD', sortOrder: 4 });
    const swaadNonVegCategory = await MenuCategory.create({ name: 'Non-Veg Specialities', section: 'SWAAD', sortOrder: 5 });
    const swaadBreadCategory = await MenuCategory.create({ name: 'Tandoor Breads', section: 'SWAAD', sortOrder: 6 });
    const swaadChineseCategory = await MenuCategory.create({ name: 'Chinese Corner', section: 'SWAAD', sortOrder: 7 });
    const swaadDessertCategory = await MenuCategory.create({ name: 'Desserts & Beverages', section: 'SWAAD', sortOrder: 8 });
    ...
    // --- LIQUID LOUNGE BAR ---
    const llbWhiskyCategory = await MenuCategory.create({ name: 'Premium Whisky', section: 'LIQUID_LOUNGE', sortOrder: 1 });
    const llbBrandyCategory = await MenuCategory.create({ name: 'Brandy', section: 'LIQUID_LOUNGE', sortOrder: 2 });
    const llbBeerCategory = await MenuCategory.create({ name: 'Beer', section: 'LIQUID_LOUNGE', sortOrder: 3 });
    const llbCocktailCategory = await MenuCategory.create({ name: 'Cocktails & Mocktails', section: 'LIQUID_LOUNGE', sortOrder: 4 });
    const llbScotchCategory = await MenuCategory.create({ name: 'Scotch & Single Malt', section: 'LIQUID_LOUNGE', sortOrder: 5 });
